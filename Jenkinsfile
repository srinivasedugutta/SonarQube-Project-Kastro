pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'srinivasedugutta'  // Set this directly to your Docker Hub username
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/srinivasedugutta/SonarQube-Project-Kastro.git'
            }
        }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=srini -Dsonar.projectKey=srinikey -Dsonar.java.binaries=target"
                }
            }
        }
